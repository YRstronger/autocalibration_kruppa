...
namespace Simple_OrbSlam {
	...
	bool OrbMatching::MatchByProject(std::vector<cv::Point3d>& vp3d1, cv::Mat& desp1,
		...
		for (int i = 0; i < vp3d1.size(); ++i) {
			cv::Mat p3d = (cv::Mat_<double>(3, 1) << vp3d1[i].x, vp3d1[i].y, vp3d1[i].z);   //地图三维点
			cv::Mat p3d_trans = R * p3d + t;    //变换
			p3d_trans /= p3d_trans.at<double>(2, 0);   //归一化
			double u = K.at<double>(0, 0)*p3d_trans.at<double>(0, 0) + K.at<double>(0, 2);
			double v = K.at<double>(1, 1)*p3d_trans.at<double>(1, 0) + K.at<double>(1, 2);     //投影到像素平面
