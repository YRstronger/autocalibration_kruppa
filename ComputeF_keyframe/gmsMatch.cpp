...
GMSMatcher::GMSMatcher(const vector<KeyPoint>& vkp1, const Size& size1, const vector<KeyPoint>& vkp2, const Size& size2,
	const vector<DMatch>& vDMatches, const double thresholdFactor) : mThresholdFactor(thresholdFactor)
{
	...
	pointIdxInPerCell.resize(mGridNumberLeft);
...
void GMSMatcher::assignMatchPairs(const int gridType,const int rotationtype)
{
    for (size_t i = 0; i < mNumberMatches; i++)
    {
		...
		//added by 睿
		//获得每个cell中的点的匹配索引
		if (gridType == 1 && rotationtype == 1) {
			if (lgidx != -1) {
				pointIdxInPerCell[lgidx].push_back(i);
			}
		}
...
int GMSMatcher::run(const int rotationType)
{
    ...
    for (int gridType = 1; gridType <= 4; gridType++)
    {
        ...
        assignMatchPairs(gridType, rotationType);
        verifyCellPairs(rotationType);

        // Mark inliers
		for (size_t i = 0; i < mNumberMatches; i++)
		{
			if (i > (mvMatchPairs.size() - 1))
				break;

			if (mvMatchPairs[i].first == -1) {
				continue;
			}
			else
			{
	...
	const double thresholdFactor)
{
	...
	if (EvenFlag)
	{
		vector<vector<int>> pointInCell;
		pointInCell = gms.pointIdxInPerCell;


		for (int j = 0; j < pointInCell.size(); ++j)
		{
			vector<int> cellmatch = pointInCell[j];
			if (cellmatch.size() <= 2) { continue; }
			else
			{
				int breaknum = 0;
				for (int k = 0; k < cellmatch.size(); ++k)
				{
					if (breaknum > 1)
					{
						inlierMask[cellmatch[k]] = false;
						continue;
					}
					if (matches1to2[cellmatch[k]].distance < 32.0 && inlierMask[cellmatch[k]] == true)
					{
						breaknum++;
					}
